...
a configuration file for building a Twarp installer.
...
* Run `cargo build` to ensure the installer uses the latest version of Twarp.
...
```shell
.\script\windows\Output\TwarpDevSetup.exe
```
...
* `MyAppExeName` (default: `twarp-dev.exe`)
