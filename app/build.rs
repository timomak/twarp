...
fn main() -> Result<()> {
    ...
    if target_os == "macos" && target_family != "wasm" {
        ...
        println!("cargo:rerun-if-changed=DockTilePlugin/TwarpDockTilePlugin.m");
        println!("cargo:rerun-if-changed=DockTilePlugin/TwarpDockTilePlugin.h");
        ...
        let plugin_src = Path::new("DockTilePlugin/TwarpDockTilePlugin.docktileplugin");
        let plugin_dst = target_dir.join("TwarpDockTilePlugin.docktileplugin");
