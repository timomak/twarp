...
use twarpui::{
    AppContext, Entity, EntityId, ModelContext, ModelSpawner, SingletonEntity, ViewHandle,
};
...
impl BrowserMcpBridge {
    ...
    /// fallback: an unscoped endpoint can't target the calling session's
    /// tab, so on failure the server is simply not injected.
    pub(crate) fn codex_config_json_for_session(&self, session_id: &str) -> Option<String> {
        let runtime = self.server.as_ref()?;
        let url = runtime
            .session_http_server_url(session_id)
            .inspect_err(|err| {
                log::warn!("Failed to start session-scoped browser MCP HTTP server: {err}");
            })
            .ok()?;
...
impl BrowserMcpRuntime {
    ...
    fn session_http_server_url(&self, session_id: &str) -> Result<String, String> {
        ...
        let url = crate::mcp_streamable_http::start_streamable_http_server(
            ...
            self.cancel.child_token(),
            "twarp browser",
            move || BrowserMcpServer::new(spawner.clone(), Some(scope.clone())),
        )?;
        servers.insert(session_id.to_owned(), url.clone());
        Ok(url)
    }
