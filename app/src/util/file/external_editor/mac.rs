...
pub fn open_file_path_with_line_and_col(
    ...
    if full_path.is_file() {
        ...
        // Twarp channel (e.g. Stable handling files while Preview is running).
        // When the resolved default is a different Twarp, open with the
...
fn is_twarp_bundle(bundle_id: &str) -> bool {
    AppId::parse(bundle_id)
        .map(|id| id.qualifier() == "dev" && id.organization() == "twarp")
        .unwrap_or(false)
}
