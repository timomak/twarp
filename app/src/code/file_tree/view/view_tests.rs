...
use twarpui::{platform::WindowStyle, App, ModelHandle, SingletonEntity};
...
fn build_repo_state(repo_root: &std::path::Path) -> FileTreeState {
    ...
    let src_dir = Entry::Directory(DirectoryEntry {
        path: twarp_util::standardized_path::StandardizedPath::try_from_local(
...
fn repo_transition_unregisters_lazy_loaded_path() {
    VirtualFS::test("file_tree_repo_transition", |dirs, mut vfs| {
        ...
        let canonical_repo_root =
            twarp_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_root)
                .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &twarp_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &twarp_util::standardized_path::StandardizedPath::try_from_local(&src_dir)
                        .unwrap(),
                    ctx,
                );
            });
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &twarp_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &twarp_util::standardized_path::StandardizedPath::try_from_local(&nested_dir)
                        .unwrap(),
                    ctx,
                );
            });
