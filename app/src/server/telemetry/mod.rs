...
use twarp_core::channel::RudderStackDestination;
use twarpui::telemetry::Event;
...
pub fn clear_event_queue() {
    let _ = twarpui::telemetry::flush_events();
}
...
impl TelemetryApi {
    ...
    pub async fn flush_events(&self, settings_snapshot: PrivacySettingsSnapshot) -> Result<usize> {
        let events = twarpui::telemetry::flush_events();
    ...
    fn persist_events_to_telemetry_log_file(&self, events: Vec<Event>) -> Result<()> {
        let log_directory = twarp_logging::log_directory()?;
    ...
    pub async fn send_telemetry_event(
        ...
        anonymous_id: String,
        event: impl twarp_core::telemetry::TelemetryEvent,
        settings_snapshot: PrivacySettingsSnapshot,
    ) -> Result<()> {
        let event = twarpui::telemetry::create_event(
