...
pub fn is_crash_recovery_process(args: &twarp_cli::AppArgs) -> bool {
...
impl CrashRecovery {
    ...
    #[cfg(test)]
    pub fn register_for_test(app: &mut twarpui::App) {
        ...
        app.update(|ctx| {
            ctx.add_singleton_model(|ctx| {
                ...
                let launch_mode = crate::LaunchMode::App {
                    args: twarp_cli::AppArgs::default(),
                    api_key: None,
                };
...
#[cfg(windows)]
fn wait_for_parent_crash(args: &twarp_cli::AppArgs) {
    use windows::Win32::{
...
fn handle_parent_crash(
    ...
    twarp_logging::on_parent_process_crash();
