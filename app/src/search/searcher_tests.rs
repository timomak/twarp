...
fn test_tokenizer_warp_special_chars() {
    ...
    let test_string = "warp-cli/launch_command:run C:\\\\Program_Files\\\\Twarp\\\\core-engine.dll check_status:/dev/twarp_drive-0";
    ...
    assert_token(
        &tokens[7],
        7,
        "C:\\\\Program_Files\\\\Twarp\\\\core-engine",
        28,
        64,
    );
    assert_token(&tokens[15], 15, "dll", 65, 68);
    assert_token(&tokens[16], 16, "check_status:/dev/twarp_drive-0", 69, 99);
}
