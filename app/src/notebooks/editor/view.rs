...
use twarp_util::{path::LineAndColumnArg, user_input::UserInput};
use twarpui::{
...
use twarpui::{actions::StandardAction, elements::Hoverable};
use twarpui::{keymap::PerPlatformKeystroke, windowing::WindowManager};
...
use crate::{
    ...
    util::{
        ...
        tooltips::{render_tooltip, should_show_open_in_twarp_link, TooltipLink, TooltipRedaction},
    },
...
pub enum EditorViewAction {
    ...
    InsertBlock(twarp_editor::content::text::BlockType),
    Indent,
    Unindent,
    Tab,
    ShiftTab,
    ...
    MiddleClickPaste,
    /// Open a file. If open_in_twarp is true, open in Warp's code editor; otherwise use external editor.
    OpenFile {
        path: PathBuf,
        line_and_column_num: Option<LineAndColumnArg>,
        force_open_in_twarp: bool,
    },
...
struct FilePathMouseStateHandles {
    ...
    open_in_twarp_handle: MouseStateHandle,
}
...
impl RichTextEditorView {
    ...
    pub(super) fn insert_block(
        &mut self,
        block_type: twarp_editor::content::text::BlockType,
        ctx: &mut ViewContext<Self>,
    ) {
        enum InsertionMode {
            ...
            self.model.update(ctx, |model, ctx| {
                match insertion_mode {
                    ...
                    InsertionMode::DeleteSlashAndRestyleLine(cursor_position) => match block_type {
                        twarp_editor::content::text::BlockType::Item(item) => {
                        ...
                        twarp_editor::content::text::BlockType::Text(style) => {
                            // Note: We can use `set_last_selection_head` because the menu cannot
                            // be opened when there are multiple selections.
    ...
    fn render_file_path_tooltip(
        ...
        .to_string();
        let show_open_in_twarp = should_show_open_in_twarp_link(&path, ctx);
        ...
        if show_open_in_twarp {
            let path_for_warp = path.clone();
            links.push(TooltipLink {
                text: "Open in Twarp".to_string(),
                on_click: Box::new(move |ctx: &mut EventContext| {
                    ctx.dispatch_typed_action(EditorViewAction::OpenFile {
                        path: path_for_warp.clone(),
                        line_and_column_num,
                        force_open_in_twarp: true,
                    });
                }),
                detail: None,
                mouse_state: self.file_path_mouse_states.open_in_twarp_handle.clone(),
            });
        }
...
impl View for RichTextEditorView {
    ...
    fn active_cursor_position(&self, ctx: &ViewContext<Self>) -> Option<twarpui::CursorInfo> {
        ...
        match action {
            ...
            OpenFile {
                ...
                ctx.emit(EditorViewEvent::OpenFile {
                    ...
                    force_open_in_twarp: *force_open_in_twarp,
                });
            }
        }
...
impl twarp_editor::editor::EditorView for RichTextEditorView {
