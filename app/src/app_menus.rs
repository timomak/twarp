...
const ENABLE_PTY_RECORDING: &str = "Enable PTY Recording Mode (twarp.pty.recording)";
const DISABLE_PTY_RECORDING: &str = "Disable PTY Recording Mode (twarp.pty.recording)";
...
const SHOW_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "Show Twarpified SSH Blocks";
const HIDE_SSH_COMMAND_BLOCKS_MENU_ITEM_NAME: &str = "Hide Twarpified SSH Blocks";
const EXPORT_DEFAULT_SETTINGS_CSV_MENU_ITEM_NAME: &str =
    "Export Default Settings as CSV to home dir";

const SETTINGS_CSV_FILE_NAME: &str = "twarp_default_settings.csv";
...
fn make_new_help_menu() -> Menu {
    Menu::new(
        ...
        vec![
            ...
            link_menu_item("Upstream Warp Slack Community...", links::SLACK_URL.into()),
        ],
    )
}
...
fn make_new_elements_menu_items(ctx: &AppContext) -> Vec<MenuItem> {
    ...
    let mut new_elements_menu = vec![
        ...
        // twarp: AI is permanently disabled. The "New Terminal Tab" branch
        // upstream toggled between NewTab / NewTerminalTab depending on the
        // user's default session mode (Agent vs Terminal); with AI off the
        // agent branch is unreachable, so the menu always dispatches NewTab.
        // The "New Agent Tab" item is removed.
        MenuItem::Custom(CustomMenuItem::new(
            "New Terminal Tab",
