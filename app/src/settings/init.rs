...
use twarpui::{rendering::GPUPowerPreference, AppContext, SingletonEntity};
...
impl CloudAgentSettings {
    pub fn register(_: &mut twarpui::AppContext) {}
}
use crate::{
    appearance,
    banner::BannerState,
...
pub fn init(
    ...
    #[cfg(feature = "local_fs")]
    {
        let prefs = <settings::PublicPreferences as twarpui::SingletonEntity>::as_ref(ctx);
...
fn handle_warp_config_change(
    _: twarpui::ModelHandle<crate::user_config::WarpConfig>,
...
fn init_platform_native_preferences() -> user_preferences::Model {
    cfg_if::cfg_if! {
        if #[cfg(test)] {
            ...
            let app_id = twarp_core::channel::ChannelState::app_id();
            ...
            Box::new(user_preferences::user_defaults::UserDefaultsPreferencesStorage::new(
                twarp_core::channel::ChannelState::data_domain_if_not_default()
            ))
...
pub fn init_public_user_preferences() -> (user_preferences::Model, Option<user_preferences::Error>)
{
    cfg_if::cfg_if! {
        if #[cfg(test)] {
            ...
            if twarp_core::features::FeatureFlag::SettingsFile.is_enabled() {
