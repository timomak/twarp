...
use twarpui::elements::ConstrainedBox;
...
impl<P: BackingView> PaneHeader<P> {
    ...
    pub fn has_shareable_object<C: twarpui::ViewAsRef>(&self, ctx: &C) -> bool {
        self.sharing_dialog().as_ref(ctx).has_target()
    }

    pub fn has_shareable_shared_session<C: twarpui::ViewAsRef>(&self, ctx: &C) -> bool {
        self.sharing_dialog()
            .as_ref(ctx)
            .has_shared_session_target()
    }

    pub fn is_sharing_dialog_enabled<C: twarpui::ViewAsRef>(&self, ctx: &C) -> bool {
