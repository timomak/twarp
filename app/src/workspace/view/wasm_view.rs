...
use twarpui::elements::{ChildView, Element};
use twarpui::{AppContext, SingletonEntity, ViewContext, ViewHandle};
...
use crate::workspace::view::{NotebookSource, OpenTwarpDriveObjectSettings, Workspace};
...
impl Workspace {
    ...
    pub(super) fn build_open_in_twarp_button(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<ActionButton> {
        ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Open in Twarp", PrimaryTheme).on_click(move |ctx| {
                ...
                if let Some(url) = parse_current_url() {
                    ...
                    log::warn!("Could not get URL for Open in Twarp button");
                }
            })
        })
    }
