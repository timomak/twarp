...
use twarpui::platform::file_picker::{FilePickerConfiguration, FilePickerError};
    ...
    fn render_inbox_callout(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let formatted = FormattedText::new([FormattedTextLine::Line(vec![
            FormattedTextFragment::plain_text(
                "Twarp pipes through notifications from any CLI coding agent into a unified notification center that works across all coding agents and harnesses. ",
            ),
            learn_more_fragment,
        ])]);
...
impl View for HoaOnboardingFlow {
    ...
    fn on_focus(&mut self, _focus_ctx: &twarpui::FocusContext, ctx: &mut ViewContext<Self>) {
        ctx.focus_self();
    }
