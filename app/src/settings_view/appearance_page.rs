...
use twarpui::fonts::{FamilyId, FontInfo, Weight};
use twarpui::keymap::{ContextPredicate, FixedBinding};
use twarpui::platform::{Cursor, FilePickerConfiguration, GraphicsBackend};
...
use twarpui::{platform::SystemTheme, Action};
use twarpui::{
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    app.register_fixed_bindings(vec![FixedBinding::empty(
        "Toggle Input Mode (Twarp/Classic)".to_string(),
        builder(SettingsAction::AppearancePageToggle(
...
impl AppearanceSettingsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<AppearanceSettingsPageView>) -> Self {
        ...
        if ChannelState::channel() != Channel::Integration {
            ...
            {
                let all_system_fonts = twarpui::fonts::Cache::handle(ctx)
    ...
    fn input_mode_dropdown_item_label(val: InputMode) -> &'static str {
        match val {
            InputMode::PinnedToBottom => "Pin to the bottom (Twarp mode)",
                ...
                return Flex::column()
                    ...
                    .with_child(
                        appearance
                            .ui_builder()
                            .wrappable_text(
                                "You may need to restart Twarp for MacOS to apply the preferred icon style.",
                                true,
                            )
...
impl SettingsWidget for ToolsPanelStateScopeWidget {
    ...
    fn search_terms(&self) -> &str {
        "left tools panel open closed across tabs file tree project explorer global search twarp drive conversation list"
    }
...
impl SettingsWidget for InputTypeWidget {
    ...
    fn search_terms(&self) -> &str {
        "input type twarp universal classic style prompt terminal ai developer mode interface shell chips ps1"
    }
...
impl SettingsWidget for PromptWidget {
    ...
    fn search_terms(&self) -> &str {
        "prompt ps1 terminal twarp shell custom"
    }
