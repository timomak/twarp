...
use {
    ...
    twarpui::platform::linux::windowing_system_is_customizable,
};
...
use crate::settings::{
    ...
    CodeEditorLineNumberMode, CodeEditorLineNumberModeSetting, CodeSettings, CommandCorrections, CompletionsOpenWhileTyping, CopyOnSelect, CtrlTabBehavior,
...
use twarp_core::semantic_selection::{
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    if DefaultTerminal::can_warp_become_default() {
        app.register_fixed_bindings([FixedBinding::empty(
            "Make Twarp the default terminal",
            builder(SettingsAction::FeaturesPageToggle(
    ...
    fn handle_action(&mut self, action: &FeaturesPageAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            ToggleForceX11 => {
                ...
                self.page.scroll_by(twarpui::units::Pixels::new(40.));
                ctx.notify();
            }
            ToggleQuitOnLastWindowClosed => {
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
...
impl SettingsPageMeta for FeaturesPageView {
    ...
    fn on_page_selected(&mut self, _: bool, ctx: &mut ViewContext<Self>) {
        // On MacOS, we rely on [`twarpui::platform::AppCallbacks::on_screen_changed`] to update and
...
impl SettingsWidget for LoginItemWidget {
    ...
    fn render(
        ...
        let ui_builder = appearance.ui_builder();
        #[cfg(target_os = "macos")]
        let label = "Start Twarp at login (requires macOS 13+)";
        #[cfg(not(target_os = "macos"))]
        let label = "Start Twarp at login";
        render_body_item::<FeaturesPageAction>(
            label.into(),
            None,
            LocalOnlyIconState::for_setting(
...
impl SettingsWidget for DefaultTerminalWidget {
    ...
    fn search_terms(&self) -> &str {
        "twarp default terminal application"
    }
    ...
    fn render(
        ...
        if default_terminal.is_warp_default() {
            ui_builder
                .wrappable_text("Twarp is the default terminal", true)
                .with_style(UiComponentStyles {
            ...
            ui_builder
                .link(
                    "Make Twarp the default terminal".to_string(),
                    None,
                    Some(Box::new(|ctx| {
...
impl SettingsWidget for SSHWrapperWidget {
    ...
    fn render(
        ...
        let ui_builder = appearance.ui_builder();
        render_body_item::<FeaturesPageAction>(
            "Twarp SSH Wrapper".into(),
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
                on_click_action: Some(FeaturesPageAction::OpenUrl(
                    "https://docs.warp.dev/terminal/twarpify/ssh-legacy#implementation".into(),
                )),
...
impl SettingsWidget for DesktopNotificationsWidget {
    ...
    fn render(
        ...
        column.add_child(render_body_item::<FeaturesPageAction>(
            "Receive desktop notifications from Twarp".into(),
            Some(AdditionalInfo {
                mouse_state: self.additional_info_link.clone(),
...
impl SettingsWidget for WindowSystemWidget {
    ...
    fn render(
        ...
        if view.force_x11_changed {
            secondary_text.push_str("\n\nRestart Twarp for changes to take effect.");
        }
