...
const CODEBASE_INDEX_DESCRIPTION: &str = "Twarp can automatically index code repositories as you navigate them, helping agents quickly understand context and provide solutions. Code is never stored on the server. If a codebase is unable to be indexed, Twarp can still navigate your codebase and gain insights via grep and find tool calling.";
const WARP_INDEXING_IGNORE_DESCRIPTION: &str = "To exclude specific files or directories from indexing, add them to the .twarpindexingignore file in your repository directory. These files will still be accessible to AI features, but they won't be included in codebase embeddings.";
const AUTO_INDEX_FEATURE_NAME: &str = "Index new folders by default";
const AUTO_INDEX_DESCRIPTION: &str = "When set to true, Twarp will automatically index code repositories as you navigate them - helping agents quickly understand context and provide targeted solutions.";
