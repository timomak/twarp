...
use twarpui::elements::{FormattedTextElement, HighlightedHyperlink};
...
use crate::terminal::twarpify::settings::{
    EnableSshWarpification, SshExtensionInstallMode, SshExtensionInstallModeSetting,
    TwarpifySettingsChangedEvent, UseSshTmuxWrapper,
};
...
use crate::{
    ...
    terminal::twarpify::settings::TwarpifySettings,
...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    // Add all of the toggle settings from the Twarpify Page that you want to show up on the Command Palette here.
    let mut toggle_binding_pairs = vec![];
    if FeatureFlag::SSHTmuxWrapper.is_enabled()
        && TwarpifySettings::as_ref(app)
            .enable_ssh_warpification
            .is_supported_on_current_platform()
    {
        toggle_binding_pairs.push(ToggleSettingActionPair::new(
            "SSH Warpification",
            builder(SettingsAction::TwarpifyPageToggle(
                TwarpifyPageAction::ToggleSshWarpification,
            )),
            context,
    ...
    if FeatureFlag::SSHTmuxWrapper.is_enabled() {
        toggle_binding_pairs.push(ToggleSettingActionPair::new(
            "SSH session detection for Twarpification",
            builder(SettingsAction::TwarpifyPageToggle(
                TwarpifyPageAction::ToggleTmuxWarpification,
            )),
            context,
...
const SSH_TMUX_WARPIFICATION_DESCRIPTION: &str = "The tmux ssh wrapper works in many situations where the default one does not, but may require you to hit a button to twarpify. Takes effect in new tabs.";

const SSH_EXTENSION_INSTALL_MODE_DESCRIPTION: &str =
    "Controls the installation behavior for Twarp's SSH extension when a remote host doesn't have it installed.";

/// This page lets users configure when they get asked to twarpify a session. Some shell commands
...
/// related to twarpification.
pub struct TwarpifyPageView {
    page: PageType<Self>,
    ...
    ssh_extension_install_mode_dropdown: ViewHandle<Dropdown<TwarpifyPageAction>>,
}

impl TwarpifyPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let twarpify_settings_handle = TwarpifySettings::handle(ctx);

        ctx.observe(&twarpify_settings_handle, Self::update_button_states);
        ctx.subscribe_to_model(&twarpify_settings_handle, move |me, model, event, ctx| {
            me.update_button_states(model, ctx);
            if matches!(
                event,
                TwarpifySettingsChangedEvent::SshExtensionInstallModeSetting { .. }
            ) {
        ...
        instance.update_button_states(twarpify_settings_handle, ctx);
        instance
    }
    ...
    fn build_page(ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        let twarpify_settings = TwarpifySettings::as_ref(ctx);
        if FeatureFlag::SSHTmuxWrapper.is_enabled()
            && twarpify_settings
                .enable_ssh_warpification
                .is_supported_on_current_platform()
        {
            categories.push(
                Category::new("SSH", vec![Box::new(SSHWidget::default())])
                    .with_subtitle("Twarpify your interactive SSH sessions."),
            );
        }
    ...
    fn update_button_states(
        &mut self,
        twarpify_settings_handle: ModelHandle<TwarpifySettings>,
        ctx: &mut ViewContext<Self>,
    ) {
        let twarpify_settings = twarpify_settings_handle.as_ref(ctx);
        self.remove_denylisted_command_button_states = twarpify_settings
            .subshell_command_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_added_command_button_states = twarpify_settings
            .added_subshell_commands
            .iter()
            .map(|_| Default::default())
            .collect();
        self.remove_denylisted_ssh_button_states = twarpify_settings
            .ssh_hosts_denylist
            .iter()
            .map(|_| Default::default())
            .collect();
    ...
    /// `TwarpifySettings::ssh_extension_install_mode` value (e.g. after it
    /// was changed from the SSH remote server choice view).
    fn update_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        let current_mode = *TwarpifySettings::as_ref(ctx)
            .ssh_extension_install_mode
            .value();
        self.ssh_extension_install_mode_dropdown
            .update(ctx, |dropdown, ctx| {
                dropdown.set_selected_by_action(
                    TwarpifyPageAction::SetSshExtensionInstallMode(current_mode),
                    ctx,
                );
            });
    }
    ...
    fn handle_added_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                TwarpifySettings::handle(ctx).update(ctx, |twarpify_settings, ctx| {
                    twarpify_settings.add_subshell_command(new_command, ctx);
                });
    ...
    fn handle_denylisted_command_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                TwarpifySettings::handle(ctx).update(ctx, |twarpify_settings, ctx| {
                    twarpify_settings.denylist_subshell_command(new_command, ctx);
                });
    ...
    fn handle_denylisted_ssh_editor_event(
        ...
        match event {
            SubmittableTextInputEvent::Submit(new_command) => {
                TwarpifySettings::handle(ctx).update(ctx, |twarpify_settings, ctx| {
                    twarpify_settings.denylist_ssh_host(new_command, ctx);
                });
    ...
    fn remove_denylisted_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        TwarpifySettings::handle(ctx).update(ctx, |twarpify, ctx| {
            twarpify.remove_denylisted_subshell_command(index, ctx)
        });
    }
    ...
    fn remove_added_command(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        TwarpifySettings::handle(ctx).update(ctx, |twarpify, ctx| {
            twarpify.remove_added_subshell_command(index, ctx)
        });
    }
    ...
    fn remove_denylisted_ssh_host(&self, index: usize, ctx: &mut ViewContext<Self>) {
        ...
        TwarpifySettings::handle(ctx).update(ctx, |twarpify, ctx| {
            twarpify.remove_denylisted_ssh_host(index, ctx)
        });
    }
}

impl Entity for TwarpifyPageView {
...
impl TwarpifyPageView {
    fn create_ssh_extension_install_mode_dropdown(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<Dropdown<TwarpifyPageAction>> {
        let items: Vec<DropdownItem<TwarpifyPageAction>> = SshExtensionInstallMode::iter()
            .map(|mode| {
                DropdownItem::new(
                    mode.display_name(),
                    TwarpifyPageAction::SetSshExtensionInstallMode(mode),
                )
            })
            .collect();
        ...
        let current_mode = *TwarpifySettings::as_ref(ctx)
            .ssh_extension_install_mode
            .value();
        let enable_ssh_warpification = *TwarpifySettings::as_ref(ctx)
            .enable_ssh_warpification
            .value();
...
impl View for TwarpifyPageView {
...
impl TypedActionView for TwarpifyPageView {
    type Action = TwarpifyPageAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        use TwarpifyPageAction::*;
        match action {
            ...
            ToggleSshWarpification => {
                TwarpifySettings::handle(ctx).update(ctx, |ssh_settings, ctx| {
                ...
                let enabled = *TwarpifySettings::as_ref(ctx)
                    .enable_ssh_warpification
                    .value();
                self.ssh_extension_install_mode_dropdown
                    .update(ctx, |dropdown, ctx| {
            ...
            SetSshExtensionInstallMode(mode) => {
                TwarpifySettings::handle(ctx).update(ctx, |twarpify_settings, ctx| {
                    report_if_error!(twarpify_settings
                        .ssh_extension_install_mode
                        .set_value(*mode, ctx));
                    send_telemetry_from_ctx!(
            ...
            TwarpifyPageAction::RemoveDenylistedSshHost(index) => {
...
impl SettingsPageMeta for TwarpifyPageView {
    fn section() -> SettingsSection {
        SettingsSection::Twarpify
    }
...
impl From<ViewHandle<TwarpifyPageView>> for SettingsPageViewHandle {
    fn from(view_handle: ViewHandle<TwarpifyPageView>) -> Self {
        SettingsPageViewHandle::Twarpify(view_handle)
    }
}
    ...
    fn render_top_of_page(&self, appearance: &Appearance, _app: &AppContext) -> Box<dyn Element> {
        let twarpify_description = vec![
            FormattedTextFragment::plain_text(
                "Configure whether Twarp attempts to “Twarpify” (add support for blocks, \
                    input modes, etc) certain shells. ",
            ),
            FormattedTextFragment::hyperlink(
                "Learn more",
                "https://docs.warp.dev/terminal/twarpify/subshells",
            ),
        ];
        ...
        Flex::column()
            .with_child(render_page_title("Twarpify", HEADER_FONT_SIZE, appearance))
            .with_child(twarpify_description)
            .finish()
    }
}

impl SettingsWidget for TitleWidget {
    type View = TwarpifyPageView;

    fn search_terms(&self) -> &str {
        "ssh subshell twarpify session"
    }
    ...
    fn render_subshells_section(
        ...
        column.add_child(
            view.build_input_list(
                "Added commands",
                &twarpify_settings.added_subshell_commands,
                &view.remove_added_command_button_states,
                TwarpifyPageAction::RemoveAddedCommand,
        ...
        column.add_child(
            view.build_input_list(
                "Denylisted commands",
                &twarpify_settings.subshell_command_denylist,
                &view.remove_denylisted_command_button_states,
                TwarpifyPageAction::RemoveDenylistedCommand,
...
impl SettingsWidget for SSHWidget {
    ...
    fn render(
        ...
        let enable_ssh_warpification = *TwarpifySettings::as_ref(app)
            .enable_ssh_warpification
            .value();

        let should_prompt_ssh_tmux_wrapper =
            *TwarpifySettings::as_ref(app).use_ssh_tmux_wrapper.value();

        add_setting(
            &mut column,
            &TwarpifySettings::as_ref(app).enable_ssh_warpification,
            move || {
                render_body_item::<TwarpifyPageAction>(
                    "Twarpify SSH Sessions".into(),
                    None,
                    ...
                    ui_builder
                        ...
                        .on_click(move |ctx, _, _| {
                            ctx.dispatch_typed_action(TwarpifyPageAction::ToggleSshWarpification);
                        })
                        .finish(),
                    None,
                )
        ...
        if FeatureFlag::SshRemoteServer.is_enabled() {
            ...
            add_setting(
                &mut column,
                &TwarpifySettings::as_ref(app).ssh_extension_install_mode,
                move || {
        ...
        add_setting(
            &mut column,
            &TwarpifySettings::as_ref(app).use_ssh_tmux_wrapper,
            move || {
                let mut column = Flex::column();

                column.add_child(render_body_item::<TwarpifyPageAction>(
                    "Use Tmux Twarpification".into(),
                    Some(AdditionalInfo {
                        mouse_state: self.additional_info_mouse_state.clone(),
                        on_click_action: Some(TwarpifyPageAction::OpenUrl(
                            "https://docs.warp.dev/terminal/twarpify/ssh".into(),
                        )),
                        secondary_text: None,
                    ...
                    ui_builder
                        ...
                        .on_click(move |ctx, _, _| {
                            ...
                            ctx.dispatch_typed_action(TwarpifyPageAction::ToggleTmuxWarpification);
                        })
                        .finish(),
                    None,
                ));
                ...
                if enable_ssh_warpification && should_prompt_ssh_tmux_wrapper {
                    ...
                    column.add_child(
                        view.build_input_list(
                            "Denylisted hosts",
                            &twarpify_settings.ssh_hosts_denylist,
                            &view.remove_denylisted_ssh_button_states,
                            TwarpifyPageAction::RemoveDenylistedSshHost,
