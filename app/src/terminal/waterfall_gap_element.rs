...
use twarpui::elements::ZIndex;
use twarpui::event::ModifiersState;
use twarpui::units::{IntoLines, IntoPixels, Pixels};
    ...
    fn dispatch_event(
        ...
        if !handled {
            ...
            handled |= match event_at_z_index {
                Some(twarpui::Event::LeftMouseDown { position, .. }) => {
                    self.mouse_down(*position, ctx)
                }
                Some(twarpui::Event::RightMouseDown { position, .. }) => {
                ...
                Some(twarpui::Event::ScrollWheel {
                    position,
                    delta,
                    precise,
...
impl ScrollableElement for WaterfallGapElement {
    fn scroll_data(&self, app: &AppContext) -> Option<twarpui::elements::ScrollData> {
