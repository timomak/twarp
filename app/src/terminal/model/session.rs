...
use twarpui::{platform::OperatingSystem, Entity, ModelContext, SingletonEntity};
...
// SessionId is defined in twarp_core and re-exported here for backward compatibility.
pub use twarp_core::SessionId;
...
pub enum SessionType {
    ...
    /// established yet.
    WarpifiedRemote { host_id: Option<twarp_core::HostId> },
}
...
impl Session {
    ...
    pub fn set_remote_host_id(&self, host_id: Option<twarp_core::HostId>) {
    ...
    pub fn home_dir(&self) -> Option<&str> {
        if cfg!(test) {
            return twarp_util::path::TEST_SESSION_HOME_DIR.as_deref();
        }
    ...
    pub fn windows_path_converter(&self) -> Option<fn(&str) -> String> {
        if self.is_wsl() {
            Some(twarp_util::path::convert_windows_path_to_wsl)
        } else if self.is_msys2() {
            Some(twarp_util::path::convert_windows_path_to_msys2)
        } else {
            None
        }
    }
