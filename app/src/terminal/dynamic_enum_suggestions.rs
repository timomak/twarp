use twarp_completer::completer::{CommandOutput, GeneratorContext};
...
fn parse_dynamic_command(output: &CommandOutput) -> anyhow::Result<Vec<String>> {
    match output.status {
        twarp_completer::completer::CommandExitStatus::Success => {
        ...
        twarp_completer::completer::CommandExitStatus::Failure => {
