use twarpui::{assets::asset_cache::AssetSource, App};
    ...
    App::test(Assets, |mut app| async move {
        ...
        app.read(|ctx| {
            assert_script_is_short_enough_mac(
                &begin_twarpify_ssh_session_command(ctx),
            ...
            assert_script_is_short_enough_mac(
                ...
                "install_tmux_and_twarpify_brew.sh",
                false,
            );
            assert_script_is_short_enough_mac(
                &get_script(
                ...
                "fish/install_tmux_and_twarpify_brew.sh",
                false,
            );

            assert_script_is_short_enough_mac(
                &twarpify_ssh_session_command("Darwin", ShellType::Zsh, ctx)
                    .expect("Should get Darwin zsh script"),
                "zsh twarpify",
                true,
            );
            assert_script_is_short_enough_mac(
                &twarpify_ssh_session_command("Darwin", ShellType::Bash, ctx)
                    .expect("Should get Darwin bash script"),
                "bash twarpify",
                true,
            );
            assert_script_is_short_enough_mac(
                &twarpify_ssh_session_command("Darwin", ShellType::Fish, ctx)
