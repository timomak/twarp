...
use crate::{
    ...
    terminal::{
        ssh::twarpify::twarpify_description,
...
pub struct TwarpifyBannerState {
    ...
    pub initialize_twarpify_keybinding: Option<Keystroke>,
    pub hover_state: MouseStateHandle,
}

impl TwarpifyBannerState {
    pub fn new(mode: WarpificationMode, initialize_twarpify_keybinding: Option<Keystroke>) -> Self {
        Self {
            mode,
            height: 0.0,
            initialize_twarpify_keybinding,
    ...
    pub fn title(&self) -> &str {
        match &self.mode {
            WarpificationMode::Ssh { .. } => "Twarpify SSH session",
            WarpificationMode::Subshell { .. } => "Twarpify subshell",
        }
    }

    pub fn action(&self) -> TerminalAction {
        match &self.mode {
            WarpificationMode::Ssh { .. } => TerminalAction::TwarpifySSHSession,
...
pub fn render_warpification_banner(
    state: &TwarpifyBannerState,
    appearance: &Appearance,
    app: &AppContext,
) -> Box<dyn Element> {
    let yes_button = render_yes_button(
        state,
        &state.initialize_twarpify_keybinding,
    ...
    let dont_ask_button = Container::new(
        appearance
            ...
            .on_click(move |ctx, _, _| {
                ctx.dispatch_typed_action(TerminalAction::DismissTwarpifyBanner(
                    remember.to_owned(),
                ));
            })
            .finish(),
    ...
    render_block_banner(
        |hover_state| {
            ...
            {
                let description = Container::new(twarpify_description(app, hyperlink_index))
