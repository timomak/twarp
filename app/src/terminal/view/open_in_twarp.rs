...
use crate::{
    ...
    terminal::{
        ...
        view::inline_banner::{OpenInTwarpBannerAction, OpenInTwarpBannerState},
    },
...
impl TerminalView {
    pub(super) fn maybe_suggest_open_in_twarp(
        &mut self,
                ...
                move |view, maybe_match, ctx| {
                    if let Some(openable_path) = maybe_match {
                        if matches!(
                            ...
                            view.suggest_open_in_twarp(openable_path, session, ctx);
                        }
                    }
                },
            );
    ...
    /// Whether or not the "Open in Twarp" banner is open.
    #[cfg(feature = "integration_tests")]
    pub fn is_open_in_twarp_banner_open(&self) -> bool {
        self.inline_banners_state.open_in_twarp_banner.is_some()
    }

    fn close_open_in_twarp_banner(&mut self, banner_id: usize) {
        self.model
            .lock()
            .block_list_mut()
    ...
    fn open_in_twarp_banner_type_dismissed(
        &self,
        ...
        match file_type {
            OpenableFileType::Markdown => {
                *general_settings.open_in_twarp_banner_dismissed_for_markdown
            }
            OpenableFileType::Code | OpenableFileType::Text => {
                *general_settings.open_in_twarp_banner_dismissed_for_code_and_text
            }
        }
    }
    ...
    fn suggest_open_in_twarp(
        &mut self,
        openable_path: OpenablePath,
        session: Arc<Session>,
        ctx: &mut ViewContext<Self>,
    ) {
        if self.open_in_twarp_banner_type_dismissed(openable_path.file_type, ctx) {
            return;
        }

        // We only show a banner for the most recent command.
        if let Some(prev_state) = &self.inline_banners_state.open_in_twarp_banner {
            self.close_open_in_twarp_banner(prev_state.id);
        }

        let banner_id = self.inline_banners_state.next_banner_id();
        self.inline_banners_state.open_in_twarp_banner = Some(OpenInTwarpBannerState::new(
            banner_id,
            openable_path,
            session,
        ));
        self.model
            ...
            .append_inline_banner(InlineBannerItem::new(
                banner_id,
                InlineBannerType::OpenInTwarp,
            ));
        ctx.notify();
    }

    pub fn handle_open_in_twarp_banner_action(
        &mut self,
        action: OpenInTwarpBannerAction,
        ctx: &mut ViewContext<Self>,
    ) {
        match action {
            OpenInTwarpBannerAction::OpenFile => {
                if let Some(banner_state) = self.inline_banners_state.open_in_twarp_banner.take() {
                    ...
                    self.close_open_in_twarp_banner(banner_state.id);
                    ctx.notify();
                }
            }
            OpenInTwarpBannerAction::LearnMore => {
                if let Some(banner_state) = &self.inline_banners_state.open_in_twarp_banner {
                        ...
                        OpenableFileType::Markdown => {
                            GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                                report_if_error!(settings
                                    .open_in_twarp_banner_dismissed_for_markdown
                                    .set_value(true, ctx));
                            });
                        }
                        OpenableFileType::Code | OpenableFileType::Text => {
                            GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                                report_if_error!(settings
                                    .open_in_twarp_banner_dismissed_for_code_and_text
                                    .set_value(true, ctx));
                            });
                        }
                    }
    ...
    pub fn open_in_twarp_banner_accessibility_content(
        &self,
        action: OpenInTwarpBannerAction,
    ) -> ActionAccessibilityContent {
        match action {
            OpenInTwarpBannerAction::OpenFile => {
                match &self.inline_banners_state.open_in_twarp_banner {
                    Some(banner_state) => {
                        ActionAccessibilityContent::Custom(AccessibilityContent::new_without_help(
                            format!("Open {} in Twarp", banner_state.target.path.display()),
            ...
            OpenInTwarpBannerAction::LearnMore => {
                ActionAccessibilityContent::Custom(AccessibilityContent::new(
                    "Learn more",
                    "Learn more about opening Markdown files in Twarp",
                    WarpA11yRole::UserAction,
                ))
            }
        }
