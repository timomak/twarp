...
use crate::{
    ...
    terminal::{
        ...
        view::{open_in_twarp::OpenablePath, InlineBannerId, TerminalAction},
    },
...
pub enum OpenInTwarpBannerAction {
    OpenFile,
    LearnMore,
    Close,
}

pub struct OpenInTwarpBannerState {
...
/// Given an openable file, format a file-specific title for the Open in Twarp banner.
fn file_title_text(openable_path: &OpenablePath) -> String {
    match openable_path.file_type {
        OpenableFileType::Markdown => {
            "Did you know that Twarp can directly display Markdown files?".to_string()
        }
            ...
            cfg_if::cfg_if! {
                if #[cfg(not(target_family = "wasm"))] {
                    ...
                    match language.as_ref().map(|language| language.display_name()) {
                        Some(display_name) => {
                            format!("Did you know that Twarp can directly edit {display_name} files?")
                        }
                        None => "Did you know that Twarp can directly edit code?".to_string(),
                    }
                } else {
                    // The `languages` crate is not available on WASM, so use a fallback message.
                    "Did you know that Twarp can directly edit code?".to_string()
                }
            }
        }
    }
...
pub fn render_open_in_twarp_banner(
    state: &OpenInTwarpBannerState,
    view_id: EntityId,
    appearance: &Appearance,
) -> Box<dyn Element> {
    let button_text = match state.target.file_type {
        OpenableFileType::Markdown => "View in Twarp",
        OpenableFileType::Code | OpenableFileType::Text => "Edit in Twarp",
    };
    ...
    let open_button = InlineBannerTextButton {
        ...
        button_state: InlineBannerButtonState {
            on_click_event: TerminalAction::OpenInTwarpBanner(OpenInTwarpBannerAction::OpenFile),
        ...
        position_id: Some(format!("open_in_twarp_banner_button_{view_id}")),
    ...
    let learn_more_button = InlineBannerTextButton {
        ...
        text_color: appearance.theme().active_ui_text_color().into_solid(),
        button_state: InlineBannerButtonState {
            on_click_event: TerminalAction::OpenInTwarpBanner(OpenInTwarpBannerAction::LearnMore),
    ...
    let close_button = InlineBannerCloseButton(InlineBannerButtonState {
        on_click_event: TerminalAction::OpenInTwarpBanner(OpenInTwarpBannerAction::Close),
