...
use crate::terminal::{
    ...
    shell::ShellType,
    ssh::{error::SshErrorBlock, install_tmux::SshInstallTmuxBlock, twarpify::SshTwarpifyBlock},
    TerminalModel, TerminalView,
};
use std::{collections::HashMap, sync::Arc};

use super::success_block::TwarpifySuccessBlock;
...
pub enum SshBlockState {
    Twarpifying {
        handle: ViewHandle<SshTwarpifyBlock>,
    },
    TwarpifySuccess {
        handle: ViewHandle<TwarpifySuccessBlock>,
    },
    InstallTmux {
...
impl SshBlockState {
    ...
    pub fn get_block_view_id(&self) -> EntityId {
        match self {
            SshBlockState::Twarpifying { handle, .. } => handle.id(),
            SshBlockState::TwarpifySuccess { handle, .. } => handle.id(),
    ...
    pub fn collapse_script(&mut self, ctx: &mut ViewContext<TerminalView>) -> bool {
        match self {
            ...
            SshBlockState::Twarpifying { .. } => false,
            SshBlockState::TwarpifySuccess { .. } => false,
    ...
    pub fn focus(&mut self, ctx: &mut ViewContext<TerminalView>) {
        match self {
            SshBlockState::Twarpifying { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
            SshBlockState::TwarpifySuccess { .. } => {}
            SshBlockState::InstallTmux { handle } => {
                handle.update(ctx, |block, ctx| block.focus(ctx));
            }
    ...
    pub fn get_system_details(&self, app: &AppContext) -> Option<SystemDetails> {
        match self {
            ...
            SshBlockState::Twarpifying { .. } => None,
            SshBlockState::TwarpifySuccess { .. } => None,
    ...
    pub fn on_warpified_session_complete(
        ...
        match self {
            SshBlockState::InstallTmux { .. } | SshBlockState::Twarpifying { .. } => {
                let block_id = self.get_block_view_id();
                return Some(block_id);
            }
            SshBlockState::TwarpifySuccess { handle } => {
...
#[derive(Default)]
struct TwarpifyTriggerState {
    block_id: Option<BlockId>,

    /// Lets us abort an attempt to auto twarpify if the subshell command
    /// hasn't completed.
    auto_twarpify_abort_handle: Option<SpawnedFutureHandle>,
...
#[derive(Default)]
pub struct TwarpifyState {
    session_id: Option<SessionId>,

    pending_state: Option<TwarpifyTriggerState>,
...
impl TwarpifyState {
    ...
    pub fn add_auto_twarpify_abort_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.auto_twarpify_abort_handle = Some(spawned_future_handle);
    }

    pub fn abort_auto_twarpify(&mut self) {
        if let Some(abort_handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.auto_twarpify_abort_handle.take())
        {
            abort_handle.abort();
        };
    }

    pub fn add_ssh_twarpify_timeout_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.ssh_twarpify_timeout_handle = Some(spawned_future_handle);
    }

    pub fn abort_ssh_twarpify_timeout(&mut self) {
        self.replace_timeout_id();
        if let Some(handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.ssh_twarpify_timeout_handle.take())
        {
            handle.abort();
        };
    }
    ...
    pub fn get_pending_ssh_host(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &TwarpifyTriggerState| state.pending_twarpify_ssh_host.clone())
    }

    pub fn get_pending_ssh_command(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &TwarpifyTriggerState| state.pending_command.clone())
    }

    pub fn take_pending_ssh_host(&mut self) -> Option<String> {
        self.pending_state
            .as_mut()
            .and_then(|state: &mut TwarpifyTriggerState| state.pending_twarpify_ssh_host.take())
    }

    pub fn clear_pending_ssh_host(&mut self) {
        if let Some(ref mut pending_state) = self.pending_state.as_mut() {
            pending_state.pending_twarpify_ssh_host = None;
        }
    }
    ...
    pub fn set_pending_ssh_host(&mut self, command: String, ssh_host: Option<String>) {
        ...
        pending_state.pending_twarpify_ssh_host = ssh_host;
    }
    ...
    /// and we have a Twarpify Success block.
    fn on_warpified_session_complete(
        &mut self,
        state: TwarpifyTriggerState,
        ctx: &mut ViewContext<TerminalView>,
    ) -> Option<EntityId> {
