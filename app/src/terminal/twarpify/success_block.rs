...
use super::settings::TwarpifySettings;
use super::{render, subshell_bootstrap_success_block_bytes, WarpificationSource};

const VERTICAL_TEXT_MARGIN: f32 = 16.;

#[derive(Debug, Clone)]
pub enum TwarpifySuccessBlockEvent {
    OpenTwarpifySettings,
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub enum TwarpifySuccessBlockAction {
    ClearAutoTwarpifySnippet,
    OpenTwarpifySettings,
    OpenUrl(String),
}

struct AutoTwarpifySnippet {
    /// On subshell initialization, this will contain the output grid to display,
    /// containing info like how to auto-twarpify the subshell.
...
pub struct TwarpifySuccessBlock {
    source: WarpificationSource,
    spawning_command: String,
    learn_more_link_mouse_states: MouseStateHandle,
    auto_twarpify_snippet: Option<AutoTwarpifySnippet>,
}

impl TwarpifySuccessBlock {
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&TwarpifySettings::handle(ctx), move |_, _, _, ctx| {
            ctx.notify();
        });
        ...
        let auto_twarpify_snippet = auto_twarpify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoTwarpifySnippet {
        ...
        Self {
            ...
            spawning_command,
            auto_twarpify_snippet,
        }
    }

    pub fn selected_text(&self) -> Option<String> {
        self.auto_twarpify_snippet
            .as_ref()
    ...
    fn render_learn_more_link(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        appearance
            ...
            .link(
                ...
                Some(Box::new({
                    move |ctx| {
                        ctx.dispatch_typed_action(TwarpifySuccessBlockAction::OpenUrl(
                            url.to_owned(),
                        ));
                    }
                })),
    ...
    pub fn on_warpified_session_complete(&mut self, ctx: &mut ViewContext<Self>) {
        self.clear_auto_twarpify_snippet(ctx);
    }

    pub fn clear_auto_twarpify_snippet(&mut self, ctx: &mut ViewContext<Self>) {
        self.auto_twarpify_snippet = None;
        ctx.notify();
    }
    ...
    pub fn render_output_grid(
        ...
        let auto_twarpify_snippet = self.auto_twarpify_snippet.as_ref()?;

        if auto_twarpify_snippet.output_grid.is_empty() {
            return None;
        }

        let shell_language = ProgrammingLanguage::Shell(auto_twarpify_snippet.shell_type);
        let runnable_command = render_runnable_code_snippet(
            &auto_twarpify_snippet.output_grid,
            if auto_twarpify_snippet.can_write_to_rc {
                Some(&shell_language)
            } else {
                None
            },
            Some(Box::new({
                move |code_snippet: String, ctx: &mut twarpui::EventContext| {
                    ...
                    ctx.dispatch_typed_action(TwarpifySuccessBlockAction::ClearAutoTwarpifySnippet);
                }
            })),
            Some(Box::new({
                move |code_snippet: String, ctx: &mut twarpui::EventContext| {
            ...
            Some(auto_twarpify_snippet.code_snippet_handles.clone()),
            app,
        );

        let semantic_selection = SemanticSelection::as_ref(app);
        let selected_text = auto_twarpify_snippet.selected_text.clone();

        // TODO(Simon): Implement full selection and copying functionality for the TwarpifySuccessBlock.
        ...
        let output_grid = SelectableArea::new(
            auto_twarpify_snippet.selection_handle.clone(),
        ...
        let output_grid = Flex::column()
            .with_child(
                Container::new(
                    Text::new(
                        auto_twarpify_snippet.description.clone(),
...
impl Entity for TwarpifySuccessBlock {
    type Event = TwarpifySuccessBlockEvent;
}

pub const TWARPIFY_SUCCESS_BLOCK_VISIBLE_KEY: &str = "TwarpifySuccessBlockVisible";

impl View for TwarpifySuccessBlock {
    fn ui_name() -> &'static str {
        "TwarpifySuccessBlock"
    }
...
impl TypedActionView for TwarpifySuccessBlock {
    type Action = TwarpifySuccessBlockAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            TwarpifySuccessBlockAction::OpenTwarpifySettings => {
                ctx.emit(TwarpifySuccessBlockEvent::OpenTwarpifySettings);
            }
            TwarpifySuccessBlockAction::OpenUrl(url) => {
                ctx.open_url(url);
            }
            TwarpifySuccessBlockAction::ClearAutoTwarpifySnippet => {
                self.clear_auto_twarpify_snippet(ctx);
            }
        }
    }
