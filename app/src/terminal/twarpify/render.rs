...
const TWARP_DRIVE_ENV_VAR_COLLECTION_ICON_COLOR: u32 = 0xC464FFFF;
const ICON_MARGIN: f32 = 4.;
const TERMINAL_ICON: &str = "bundled/svg/terminal.svg";
pub const HORIZONTAL_TEXT_MARGIN: f32 = 20.;
pub const SSH_DOCS_URL: &str = "https://docs.warp.dev/terminal/twarpify/ssh";
pub const SUBSHELL_DOCS_URL: &str = "https://docs.warp.dev/terminal/twarpify/subshells";
...
/// Renders a "Never Twarpify this host" link or nothing.
pub fn render_never_twarpify_ssh_link(
    ssh_host: &Option<String>,
    app: &AppContext,
    appearance: &Appearance,
    mouse_state_handle: MouseStateHandle,
    on_never_twarpify: fn(&mut EventContext<'_>, ssh_host: String),
) -> Option<Box<dyn Element>> {
    let Some(ssh_host) = ssh_host else {
        return None;
    };

    let settings = TwarpifySettings::handle(app);
    if settings.as_ref(app).is_ssh_host_denylisted(ssh_host) {
        // Should only happen if user manually attempts to Twarpify a denylisted host.
        return None;
    }
    ...
    let link = appearance
        ...
        .link(
            ...
            Some(Box::new({
                ...
                move |ctx| on_never_twarpify(ctx, ssh_host.to_owned())
            })),
            mouse_state_handle,
        )
        .soft_wrap(false)
...
fn get_subshell_flag_info(subshell_source: &SubshellSource, theme: &WarpTheme) -> (String, Fill) {
    match subshell_source {
        SubshellSource::EnvVarCollection(environment_name) => (
            ...
            Fill::Solid(ColorU::from_u32(TWARP_DRIVE_ENV_VAR_COLLECTION_ICON_COLOR)),
        ),
