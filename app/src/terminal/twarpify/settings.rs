...
use twarpui::{AppContext, ModelContext};
use twarpui::{Entity, SingletonEntity};

use crate::terminal::ssh::util::{parse_interactive_ssh_command, SshTwarpifyCommand};

// Cannot directly use Vec<Regex> here b/c Regex doesn't impl Eq, Serialize, and Deserialize.
maybe_define_setting!(AddedSubshellCommands, group: TwarpifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "twarpify.subshells.added_subshell_commands",
    description: "Additional regex patterns for commands that should be recognized as subshells.",
});

maybe_define_setting!(SubshellCommandsDenylist, group: TwarpifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "twarpify.subshells.subshell_commands_denylist",
    description: "Commands that should not trigger the subshell warpification prompt.",
});

maybe_define_setting!(SshHostsDenylist, group: TwarpifySettings, {
    type: Vec<String>,
    default: Vec::new(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "twarpify.ssh.ssh_hosts_denylist",
    description: "SSH hosts that should not trigger the warpification prompt.",
});

maybe_define_setting!(EnableSshWarpification, group: TwarpifySettings, {
    type: bool,
    default: true,
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "twarpify.ssh.enable_ssh_warpification",
    description: "Whether to enable Twarp features in SSH sessions.",
});

maybe_define_setting!(UseSshTmuxWrapper, group: TwarpifySettings, {
    type: bool,
    default: false,
    supported_platforms: SupportedPlatforms::OR(SupportedPlatforms::MAC.into(), SupportedPlatforms::LINUX.into()),
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "twarpify.ssh.use_ssh_tmux_wrapper",
...
maybe_define_setting!(SshExtensionInstallModeSetting, group: TwarpifySettings, {
    type: SshExtensionInstallMode,
    default: SshExtensionInstallMode::default(),
    supported_platforms: SupportedPlatforms::ALL,
    sync_to_cloud: SyncToCloud::Globally(RespectUserSyncSetting::Yes),
    private: false,
    toml_path: "twarpify.ssh.ssh_extension_install_mode",
    description: "Controls SSH extension installation behavior.",
});
...
pub struct TwarpifySettings {
    ...
    /// A list of commands that we shouldn't attempt to twarpify. These can be added either b/c the
    /// "don't ask again" button was clicked in the trigger banner, or it was added explicitly on
    /// the Twarpify settings page. This represents the raw, serialized value.
    ...
    /// A list of hosts that we shouldn't attempt to twarpify. This supports regex.
    /// These can be added either b/c the "don't ask again" button was clicked in the trigger banner,
    /// or it was added explicitly on the Twarpify settings page.
    /// While this could live in the `SshSettings` group, the custom processing shared with the other
    /// subshell logic better justifies it living in the `TwarpifySettings` group.
    ...
    /// This setting controls whether we should ever twarpify ssh sessions.
    pub enable_ssh_warpification: EnableSshWarpification,

    /// This setting controls whether we should prompt the user to twarpify an ssh session using the
...
/// define_settings_group!(TwarpifySettings). Since we didn't use that macro we define it manually
/// here. It's the event emitted by the setter methods when a setting value changes.
pub enum TwarpifySettingsChangedEvent {
    AddedSubshellCommands {
...
impl Entity for TwarpifySettings {
    type Event = TwarpifySettingsChangedEvent;
}

impl SingletonEntity for TwarpifySettings {}
...
impl TwarpifySettings {
    ...
    pub fn is_compatible_subshell_command(&self, command: &str, shell_family: ShellFamily) -> bool {
        ...
        if !self.use_ssh_tmux_wrapper.value()
            && SshTwarpifyCommand::matches(command)
        ...
        // While in-band generators are our best option for twarpifying ssh sessions from powershell, hard-code
        // the twarpify subshell banner to show up.
    ...
    pub fn add_subshell_command(&mut self, command_to_add: &str, ctx: &mut ModelContext<Self>) {
        ...
        // care of emitting the TwarpifySettingsChangedEvent::AddedSubshellCommands event to keep
    ...
    /// Check if the user has asked us to remember a command and avoid asking to twarpify a subshell.
