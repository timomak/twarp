    ...
    A: Action + Clone,
{
    ...
    let text = "You are currently offline. An internet connection is required to use Twarp for the first time.";
...
pub fn render_square_logo(appearance: &Appearance) -> Box<dyn Element> {
    let image_path = if appearance.theme().inferred_color_scheme() == ColorScheme::LightOnDark {
        "bundled/svg/twarp-logo-light.svg"
    } else {
        "bundled/svg/twarp-logo-dark.svg"
    };
    ...
    A: Action + Clone,
{
    ...
    let paragraph_1 = "All of Twarp’s non-cloud features work offline.";
    let paragraph_2 = "However, we require users to be online when using Twarp for the first time in order to enable cloud features.";
    let paragraph_3 = "We offer cloud features to all users, and so we need an internet connection to meter AI usage, prevent abuse, and associate cloud objects with users. If you opt to use Twarp logged-out, a unique ID will be attached to an anonymous user account in order to support these features.";
...
pub fn render_privacy_settings_toggles<A: Action + Clone + 'static>(
    ...
    let telemetry_toggle = Flex::row()
        ...
        .with_child(
            Shrinkable::new(
                1.,
                render_privacy_settings_section_header("Help improve Twarp", appearance).finish(),
            )
            .finish(),
        )
        .with_child(
    ...
    let telemetry_description = render_description(
        appearance,
        "High-level feature usage data helps Twarp's product team prioritize the roadmap.".into(),
    );
    ...
    let cloud_conversation_storage_description = render_description(
        ...
        if PrivacySettings::as_ref(app).is_cloud_conversation_storage_enabled {
            "Agent conversations can be shared with others and are retained when you log in on different devices. This data is only stored for product functionality, and Twarp will not use it for analytics."
        } else {
