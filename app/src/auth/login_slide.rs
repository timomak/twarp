...
impl LoginSlideView {
    ...
    fn render_select_auth_content(&self, appearance: &Appearance) -> Vec<Box<dyn Element>> {
        ...
        let tos_line = Flex::row()
            .with_child(
                ui_builder
                    .span("By continuing, you agree to upstream Warp's ")
    ...
    fn render_skip_dialog(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let is_terminal = matches!(self.intention, OnboardingIntention::Terminal);
        let title_text = if is_terminal {
            "Are you sure you want to disable Twarp Drive?"
        } else {
        ...
        let body_text_str = if is_terminal {
            "Twarp Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:"
        } else {
            "Some cloud features require an account. By continuing, you won't have access to any of the following features:"
        };
        let body_text =
