use crate::cloud_object::extract_server_id_and_object_type_from_twarp_drive_link;
use crate::drive::OpenTwarpDriveObjectArgs;
...
pub enum WarpWebLink {
    Session,
    DriveObject(Box<OpenTwarpDriveObjectArgs>),
}
    ...
    if url.origin() == ChannelState::server_root_domain() {
        url.path_segments().and_then(|mut path_segments| {
            path_segments.next().and_then(|segment| match segment {
                "drive" => extract_server_id_and_object_type_from_twarp_drive_link(url)
