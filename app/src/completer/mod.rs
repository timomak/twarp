...
use twarp_util::path::{EscapeChar, ShellFamily};
...
impl CompletionContext for SessionContext {
    ...
    #[cfg(feature = "completions_v2")]
    fn js_context(&self) -> Option<&dyn twarp_completer::completer::JsExecutionContext> {
        self.js_ctx
            .as_ref()
            .map(|ctx| -> &dyn twarp_completer::completer::JsExecutionContext { ctx })
    }
...
fn ls_script_for_dir(directory: &TypedPath) -> Option<String> {
    ...
    let escaped_dir = twarp_util::path::ShellFamily::Posix.shell_escape(dir_str);
