            ...
            async move {
                if add_app_as_login_item {
                    match register(&value_name, &exe) {
                        Ok(()) => true,
                        Err(err) => {
                            log::warn!("Failed to register Twarp as a login item: {err}");
                            false
                        }
                    }
                } else {
                    match unregister(&value_name) {
                        ...
                        Err(err) => {
                            ...
                            log::warn!("Failed to unregister Twarp as a login item: {err}");
                        }
                    }
                    false
                }
...
/// isolated (`Twarp`, `TwarpPreview`, `TwarpDev`, etc.) so installing multiple
...
/// `C:\Program Files\Twarp\twarp.exe`) are parsed as a single executable path.
...
mod tests {
    ...
    impl ScratchSubkey {
        fn new(name: &str) -> Self {
            ...
            let path = format!(r"Software\Twarp\LoginItemTests\{suffix}");
    ...
    fn register_writes_quoted_path() {
        ...
        let exe = PathBuf::from(r"C:\Program Files\Twarp\twarp.exe");
        register_in(HKEY_CURRENT_USER, &scratch.path, "Twarp", &exe).unwrap();
        assert_eq!(
            scratch.read("Twarp").as_deref(),
            Some(r#""C:\Program Files\Twarp\twarp.exe""#)
        );
    }
    ...
    fn unregister_is_idempotent() {
        ...
        unregister_in(HKEY_CURRENT_USER, &scratch.path, "Twarp").unwrap();
        ...
        unregister_in(HKEY_CURRENT_USER, &scratch.path, "Twarp").unwrap();
        unregister_in(HKEY_CURRENT_USER, &scratch.path, "Twarp").unwrap();
        assert!(scratch.read("Twarp").is_none());
    }
    ...
    fn unregister_leaves_other_values_alone() {
        ...
        assert!(scratch.read("Twarp").is_none());
        assert_eq!(
            scratch.read("TwarpPreview").as_deref(),
    ...
    fn unregister_missing_subkey_is_ok() {
        unregister_in(
            HKEY_CURRENT_USER,
            r"Software\Twarp\LoginItemTests\does-not-exist",
            "Twarp",
        )
        .unwrap();
    }
