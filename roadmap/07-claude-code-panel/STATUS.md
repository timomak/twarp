...
**Phase:** impl-in-review (7b merged via [#71](https://github.com/timomak/twarp/pull/71); **7c — live driver — PR [#72](https://github.com/timomak/twarp/pull/72) open**; **shell-polish pass — PR [#73](https://github.com/timomak/twarp/pull/73) open**, stacked on #72 — merge #72 first)
...
**Impl PRs:** [#67](https://github.com/timomak/twarp/pull/67) — abandoned (rebuilt from primitives); **owner to close.** · [#69](https://github.com/timomak/twarp/pull/69) — 7b sidebar build, **merged** (placement-agnostic core + renderer; sidebar host superseded by #70). · [#71](https://github.com/timomak/twarp/pull/71) — **7b main-pane host, merged**: relocated the renderer into `IPaneType::ClaudeCode` (`ClaudeCodeView`/`ClaudeCodePane`), added the `claude`-at-submit terminal trigger (alias-aware: peels `command`/`builtin`/`exec` wrappers), deleted the sidebar host, render transcript at natural height (no `UniformList` clipping). · **[#72](https://github.com/timomak/twarp/pull/72) — 7c live driver (open):** real `claude_code::driver` replaces the synthetic source — `spawn_session` + a background event-pump (→ `async_channel` → `spawn_stream_local`) + a stdin-writer task; streaming indicator, Stop (SIGINT), lifecycle/teardown. Permission UI deferred to 7g. · **[#73](https://github.com/timomak/twarp/pull/73) — shell visual polish (open, stacked on #72):** baseline Claude-app frame for the already-built shell — centered max-width reading column, rounded bordered composer with context pills + a real Send/Stop button, user-bubble/assistant-flow message styling, centered zero-state heading, body font 13→14. Pure chrome; **no new card types** (7d–7f unchanged). Inserted per owner UI feedback on the 7c review (2026-06-10).
