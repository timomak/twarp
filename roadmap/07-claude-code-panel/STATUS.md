...
- [ ] **7-polish — Shell visual pass.** *(PR [#73](https://github.com/timomak/twarp/pull/73), in review; stacked on #72.)* Baseline Claude-app frame for the live-driver shell: centered max-width reading column, rounded composer (context pills + real Send/Stop button), clean message styling (user bubble / assistant flow), centered zero-state heading, typography. **Pure chrome — no new transcript card types** (those stay 7d–7f). Inserted per owner UI feedback on the 7c review (2026-06-10). Validates PRODUCT §32 (visual gate) for the built surface.
