...
Question{..}            → question card (AskUserQuestion via request_user_dialog, §24/§1)
...
- Inline `completed`-answer encoding for the `request_user_dialog` question card: today the dialog is released with `{behavior:"cancelled"}` and the picks resent as a turn (safe, no unverified wire shape). Capture the exact `question`-kind answer shape against the pinned CLI and answer the dialog directly so `AskUserQuestion` resolves in place.
