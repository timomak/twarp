...
PRODUCT.md's Timeline section (§§18–23) was written before the 5e click→split-pane pivot landed, when "the inline diff" meant the in-panel inline expansion. After 5e the diff lives in a generic `CodePane<CodeView>` split off the active tab; the language doesn't quite map onto the new structure. The 5d impl picks the most surgical interpretation:

- **Timeline section lives in the right Code Review panel, below the Changes section** — not below the diff editor in the split pane. The split pane is a generic `CodePane<CodeView>` shared across all code editing; layering a footer in there would mean extending `PaneView` with a footer slot or introducing a `DiffPane` variant. Both are invasive global changes for a feature-specific need. Keeping Timeline inside the panel scopes all changes to `app/src/code_review/` and reads naturally next to the file sidebar that drives focus.
- **Avatar uses a single accent-colored circle, not per-author email-hash coloring.** PRODUCT §19 specifies "color hashed from email"; the impl uses one consistent accent color for every avatar to keep this PR contained. Per-author hashing is a small follow-up that can ship without further plumbing.
- **PRODUCT §21 (click → commit-diff swap) is deferred to a `5d.2` follow-up PR.** Replacing the diff pane's editor content with `git show <sha>:<path>` (post) against `git show <sha>^:<path>` (base) requires either (a) a synthetic-content path through `LocalCodeEditorView` so the editor can render non-disk bytes, or (b) a brand-new `CommitDiffPane` variant. Both are real plumbing surfaces. This PR ships the Timeline list, paging, rename badge, and `↑` marker — the headline of §§18–20, §22, §23 — and `5d.2` will add the click target on top of the existing list.
- **No `[Back to working diff]` link.** It only makes sense once §21's click swap exists; deferred to `5d.2` along with it.
