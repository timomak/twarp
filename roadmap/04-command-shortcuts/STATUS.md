...
- [ ] **4c — Side-panel tab + list view + add-new button.** `ToolPanelView::Shortcuts` registered next to Global Search with a keyboard-icon toolbelt button. Panel content: header with "+ New shortcut" link + a row per registered shortcut (chord display + arrow-form summary), or an empty-state hint when nothing is configured. "+ New shortcut" appends a placeholder bound to the first unused chord from `cmd-shift-J..Z` with a `new_tab` action, saves via 4b's `save_to_disk`, and 4b's hot reload picks up the change. Covers PRODUCT §§26, 27, 28, 29 (minimal — no inline detail editor yet); users hand-edit `shortcuts.yaml` to customize the placeholder.
- [ ] **4d — Inline detail editor + delete/conflict UI.** Detail editor (keystroke-capture widget, action editor with type dropdowns and per-row reorder), inline validation surfacing per-field errors, [delete] button per row, conflict warnings, errors banner. Covers PRODUCT §§30-38. Consumes 4b's serialize/save/conflict/summary scaffolding.
