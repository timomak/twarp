...
- **`Type(text)`** → resolve the target tab's active terminal view (see §9). When this `type` is immediately followed by `press enter` **and** the target pane is at an editable prompt (`TerminalView::can_execute_command_now`), submit `text` through the normal command pipeline (`execute_command_or_set_pending`, which routes to `try_execute_command_from_source` with `CommandExecutionSource::User`) and consume the paired `press enter`. This is required so submit-time interceptions fire — above all the bare-`claude` → Claude Code pane trigger (feature 07); a raw PTY write bypasses the editor entirely and runs `claude` as the raw CLI instead of opening our pane. Otherwise (no following enter, or the prompt is busy with a foreground program) fall back to `terminal_view.write_to_pty(text.as_bytes().to_vec(), ctx)`. Long strings route through `terminal/view.rs`'s chunked-write helper to avoid the macOS PTY 1 KB bug.
