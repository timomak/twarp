...
File and crate renames are the worst case for git merges; doing this last keeps upstream cherry-picks clean for as long as possible. By the time 09 starts, 02 has already removed the AI surface (a chunk of the brand-bearing code), so there's less to rename.
...
- [x] **9b — Workspace + crate renames.** All `warp_*` → `twarp_*`, `warpui*` → `twarpui*`. Updates `Cargo.toml` workspace + every `use` import.
...
- [ ] **9e — Brand assets.** Replace logo SVGs, regenerate channel icon sets, rename files (`warp-*` → `twarp-*`), update references.
...
- Cherry-pick cost spikes after 9b — bias toward batching upstream pulls just before 9b lands.
