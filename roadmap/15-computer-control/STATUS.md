...
- [ ] **15a — Self-excluding capture spike.** Corner `NSPanel` overlay + click-through glow window; prove twarp's chrome is absent from a capture (`sharingType = .none`, else a ScreenCaptureKit exclusion-filter backend). **Highest-risk gate.**
