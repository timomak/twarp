...
- [x] **18b — codex driver.** Vendored app-server v2 protocol subset pinned to a minimum CLI version; spawn + initialize handshake; thread start/resume; event mapping to `TranscriptEvent`; `turn/interrupt` with tracked turnId; fixture replay tests; `CodexAgentBackend` flag enabled for dogfood/twarp-oss (mirrors `DesignShellV1`) plus the minimal bare-`codex` trigger (moved up from 18d) so the UX gate has a drivable entry point.
