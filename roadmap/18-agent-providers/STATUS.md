...
- [x] **18a — driver extraction.** Runtime `AgentDriver` trait (spawn/send/interrupt/answer/parse/sessions/capabilities); typed approval `Decision`; `provider` column on pane persistence (default claude); golden-transcript tests; zero behavior change.
- [ ] **18b — codex driver.** RESUME the existing branch, do not rewrite: `origin/fleet/18b` already implements this sub-phase with every UX smoke step passing except §8 restore (two from-scratch re-authors died at ~330k tokens on Azure compaction). In your worktree, `git reset --hard origin/fleet/18b` FIRST, then fix ONLY the §8 restore wiring per TECH's "§8 RESTORE — exact wiring" pointer, and re-verify the punch-list items still hold. Original scope: vendored app-server v2 protocol subset pinned to a minimum CLI version; spawn + initialize handshake; thread start/resume; event mapping to `TranscriptEvent`; `turn/interrupt` with tracked turnId; fixture replay tests; `CodexAgentBackend` flag enabled for dogfood/twarp-oss (mirrors `DesignShellV1`) plus the minimal bare-`codex` trigger (moved up from 18d) so the UX gate has a drivable entry point.
