...
Independent of 10 in principle, but result-click → open-file becomes useful only once the file-editor surface exists. The `warp_ripgrep` crate is already in the workspace but currently has no UI outside terminal commands.
...
- [x] **12b — Replace.** Replace-all flow with per-file diff preview before apply. Confirm + commit changes to disk. Undo via the editor's existing undo system where possible.
...
- Editor file open/save (lands with 10)
