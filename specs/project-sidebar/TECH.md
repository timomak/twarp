...
- `AddDefaultTab` and `OpenClaudeCodeInNewTab` share helpers that snapshot the active folder-backed project's root and selected/default colors before running the existing new-tab content path. If that path activates a distinct tab, the new `TabData` receives the snapshot so Cmd+T, Cmd+Shift+T, and the sidebar's `New session` row group it beneath the existing project. Settings, legacy-shell, rootless, and failed/no-op creation paths do not inherit a folder project.
...
- The traffic-light band renders the Projects toggle and command-search actions through the same token-sized outline-button renderer. Search dispatches the Cmd+P command palette action; the Projects header retains only project creation. The sidebar's `Search sessions…` row opens that palette prefiltered with the existing `claude:` stored-session atom. Cmd+Shift+F activates a dedicated Search right tool backed by the active pane group's existing `GlobalSearchView`; Files contains no search overlay.
