...
- **Folder-backed projects form an app-wide library; open tabs are its live window-local instances.** Every window shows its live tabs first and then unopened saved projects. Opening a saved project creates an independent tab in that window, while scratch projects remain window-local. There is no global horizontal tab strip and no replacement toolbar consuming its height. Live rows preserve the tab's ordering, color dot, rename/menu behavior, and drag behavior; chat panes may appear as compact children.
