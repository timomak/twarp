...
/// This purposefully shadows the `Action` trait from `twarpui`.
///
/// Since `twarpui` depends on this crate, we can't depend on the `twarpui::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `twarpui::Action` implementing this `Action`.
