...
/// winit events into twarpui events.
#[derive(Default)]
struct State {
...
impl EventLoop {
    ...
    /// Handles a request to close the window with the given twarpui and winit
    /// IDs.
    ...
    /// Handle events that may be handled by twarpui, or maybe not in some cases, e.g. window
    ...
    fn handle_converted_warpui_event(
        ...
        // passing the event into twarpui. Skip for touch events as drag_resize_window
        ...
        // If the app didn't handle the event, twarpui might still want to do something
    ...
    ///
    /// Converts `SoftKeyboardInput` events into twarpui `Event`s and dispatches
