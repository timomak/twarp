    ...
    futures::executor::block_on(async {
        ...
        let request = match &opts.resume_session_id {
            Some(thread_id) => protocol::thread_resume_request(
                ...
                &opts.cwd,
                opts.model.as_deref(),
                opts.codex_config_overrides.as_ref(),
...
fn permission_mapping(mode: PermissionMode) -> (&'static str, &'static str) {
    // Approval request/response UI lands in 18c. Until then, keep Codex inside
    // the sandbox selected by the existing permission mode but never ask the
    // app-server to emit an approval request that this client cannot answer.
    // A denied sandbox escape becomes a provider error instead of wedging the
    // turn indefinitely.
    match mode {
        PermissionMode::Plan => ("read-only", "never"),
        PermissionMode::Default | PermissionMode::AcceptEdits => ("workspace-write", "never"),
...
mod tests {
    ...
    #[test]
    fn permission_mapping_never_requests_unhandled_approvals() {
        assert_eq!(
            permission_mapping(PermissionMode::Plan),
            ("read-only", "never")
        );
        assert_eq!(
            permission_mapping(PermissionMode::Default),
            ("workspace-write", "never")
        );
        assert_eq!(
            permission_mapping(PermissionMode::AcceptEdits),
            ("workspace-write", "never")
        );
        assert_eq!(
            permission_mapping(PermissionMode::BypassPermissions),
