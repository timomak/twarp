...
use twarp_cli::WorkerCommand;
use twarp_core::channel::{Channel, ChannelConfig, ChannelState, OzConfig, WarpServerConfig};
use twarp_core::AppId;
...
pub fn main() -> Result<()> {
    ...
    if let Some(command) = &args.command {
        match command {
            ...
            WorkerCommand::TerminalServer(args) => {
                ...
                twarp::terminal::local_tty::server::run_terminal_server(args);
                return Ok(());
            }
    ...
    twarp::run_integration_test(driver)
}
...
fn register_tests() -> HashMap<&'static str, BoxedBuilderFn> {
    ...
    register_test!(test_open_in_twarp_banner);
