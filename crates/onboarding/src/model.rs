...
impl SelectedSettings {
    ...
    pub fn is_twarp_drive_enabled(&self) -> bool {
        match self {
            SelectedSettings::AgentDrivenDevelopment {
                ui_customization, ..
            } => ui_customization
                .as_ref()
                .map(|ui| ui.show_twarp_drive)
                .unwrap_or(true),
...
impl OnboardingStateModel {
    ...
    pub(crate) fn set_tools_panel_enabled(&mut self, enabled: bool, ctx: &mut ModelContext<Self>) {
        ...
        self.ui_customization.show_twarp_drive = enabled;
        ctx.notify();
    }
    ...
    pub(crate) fn set_show_twarp_drive(&mut self, value: bool, ctx: &mut ModelContext<Self>) {
        if self.ui_customization.show_twarp_drive == value {
            return;
        }
        send_telemetry_from_ctx!(
            OnboardingEvent::SettingChanged {
                setting: "twarp_drive".to_string(),
                value: value.to_string(),
            },
            ctx
        );
        self.ui_customization.show_twarp_drive = value;
        ctx.notify();
    }
