...
4. Determine the correct tab config directory for the user's Warp build.
   Tab configs live under `~/.twarp/tab_configs/` for standard builds. Non-stable builds use a channel-specific variant (e.g. `~/.twarp-<channel>/tab_configs/`).
   To find the correct directory, run `ls -d ~/.twarp*/` to list the available Warp data directories and pick the one that corresponds to the running build. When in doubt, ask the user which build they are using.
