...
macOS, Linux, and Windows are all supported. Platform-specific setup is handled by `./script/bootstrap`. See [TWARP.md](TWARP.md) for the full engineering guide.
...
Yes. Use whatever you like — Warp's built-in agent, Claude Code, Codex, Gemini CLI, Cursor, others, or no agent at all. The repo ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`TWARP.md`](TWARP.md)) that any harness supporting these formats can pick up.
